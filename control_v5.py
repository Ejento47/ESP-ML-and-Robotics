        ...
        for y in range(int(car_y-(distance/2)), int(car_y+(distance/2 + 1))):
            for x in range (int(car_x-(distance/2)), int(car_x+(distance/2 + 1))):
                ...
                if [new_y,new_x] not in og_obstacles:
                    ...
                    #print('kfc')
                '''      
        return more_empty_spaces

    def sensor_update(self, car_x, car_y, og_obstacles, empty_spaces, distance):
        extra_empty_spaces = self.process_sensor_data(car_x, car_y, og_obstacles, distance)
