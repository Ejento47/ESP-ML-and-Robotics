...
if __name__ == '__main__':
    ...
    car_obs = np.vstack([car_obs]) ####TO CHANGE BACK TO CAR_OBS
    ...
    while len(path) > 0: #while path is not empty
        ...
        key = cv2.waitKey(1)
        my_car.process_sensor_data(my_car.x, my_car.y, path_planner.a_star.obstacle_map, car_obs, parking1.empty_spaces, distance=20)
        ...
        while my_car.detect_empty_parking(parking1.cars.values(), parking1.empty_spaces): ###TESTTTT NEEDD TO BE REPLACED WHEN CAN PARK IS TRUE
