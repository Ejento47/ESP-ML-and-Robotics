...
from environment_v5 import Environment, Parking1
from pathplanning_V5 import  ParkPathPlanning, interpolate_b_spline_path, interpolate_path
from control_v5 import Car_Dynamics, MPC_Controller, Linear_MPC_Controller
...
if __name__ == '__main__':
    ...
    parking1 = Parking1(2,original_end) # random parking slot selection ## of can be args.parking
    ...
    #change the lst of cars to fit an actual y:x coordinate

    lstofcars = list(parking1.cars_copy)
    for cars in lstofcars:
        cars[0] = my_car.flip_xy(cars[0])
    ...
    while len(path) > 0: #while path is not empty
        ...
        my_car.sensor_update(my_car.x, my_car.y, path_planner.a_star.obstacle_map, empty_spaces, 20)
        ...
        if my_car.detect_empty_parking(lstofcars, empty_spaces):
